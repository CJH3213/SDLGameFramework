...
void TestScript::Awake()
{
	...
	StartCoroutine(CoRet_WaitSec);	// 要将CoRet注册到组件中，当组件销毁时才会自动暂停协程
...
void TestScript::Update()
{
	...
	{
		std::shared_ptr<Scene> newScene = std::make_shared<MusicPlayerScene>();
		SceneManager::LoadScene("Scene03", newScene);
	}
