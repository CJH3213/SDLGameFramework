...
void TetrisManager::GenerateBlocks(bool arr[2][4])
{
	...
	// 装载预测方块
	auto cou1 = mNextShapeGO->GetTransform().GetChildren().size();
	DestroyAllChildren(*mNextShapeGO);
	auto cou2 = mNextShapeGO->GetTransform().GetChildren().size();
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 4; j++)
		{
...
void TetrisManager::Start()
{
	...
	// 下一个形状父对象
	mNextShapeGO = GameObject::Find("NextShape");
	mNextShapeGO->GetTransform().SetPosition({ -300.0f, 0.0f });
	...
	auto cou2 = mNextShapeGO->GetTransform().GetChildren().size();
	GenerateBlocks(shapes[Random::Range(0, 5)]);
	auto cou3 = mNextShapeGO->GetTransform().GetChildren().size();
