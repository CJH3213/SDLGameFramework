...
CoRet SceneManager::CoFun_FrameUpdate()
{
	while (true)
	{
		...
		if (mActiveScene != nullptr)
		{
			...
			// 帧渲染
			//auto t11 = MyTime::Time_s();
			RenderManager::Update();
			//auto t21 = MyTime::Time_s();
			//auto t31 = t21 - t11;
			...
			for (auto& trans : mRemoveGameObjects)
			{
				...
				{
					it = std::find_if(mAddGameObjects.begin(), mAddGameObjects.end(),
						[&trans](std::shared_ptr<GameObject>& obj)
						{ // 通过比对变换组件判断是否同一个游戏对象
							return trans == obj->GetTransform();
						});

					if (it != mAddGameObjects.end())
					{
						(**it).Destroy();
						mAddGameObjects.erase(it);
					}
				}
